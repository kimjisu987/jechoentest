$(document).ready(function(){
	...
	$(".talent_comple_btn").click(function(e) {
		...
		function validateFields() {
			let inputs = [
				...
				{ input: $('#location'), errorMsg: "활동 장소를 입력하세요" },
				{ input: $('#price'), errorMsg: "수강 요금을 입력하세요" },
				{ input: $('#start_date'), errorMsg: "활동 시작일을 선택하세요" },
				{ input: $('#end_date'), errorMsg: "활동 종료일을 선택하세요" }
			];
	
			for (let { input, errorMsg } of inputs) {
				if (!form_check(input, errorMsg)) return false;
			}
	
			// 신청분야 유효성 검사
			if ($("#activity_field").val().length === 0) {
				cate_showError($("#activity_field"), "신청분야를 선택하세요");
				$("#activity_field").focus();
				return false;
			} else {
				hideError($("#activity_field"));
			}
	
			// textarea 유효성 검사
			if (!form_check($("#detail"), "내용을 입력하세요")) return false;
	
			// 활동요일 선택 유효성 검사
			if ($(".input_day_add").find("button").hasClass("day_del")) {
				hideError($(".input_day_add"));
			} else {
				yoil_showError($(".input_day_add"), "활동요일을 등록하세요");
				$(".input_day_add").focus();
				return false;
			}
	...
	function cate_showError(input, msg) {
		let error_msg = input.parents(".activity_field_box").find('.input_fields').find(".check_error");
		error_msg.text(msg).show();
	}
	// 활동요일 오류 메세지 표시
	function yoil_showError(input, msg) {
		let error_msg = input.parents(".input_day_box").find(".check_error");
		error_msg.text(msg).show();
	}
	// 오류 메시지 숨기기
	...
	// 재능활동가 권한자 유효성 검사
	$('#title_search, .activist_search').on('click', function() {
		$(this).parents(".input_fields").find('.form_check_box').find(".check_error").text("");
	});
	
	// 셀렉트 클릭 시 에러메세지 삭제
	$('#activity_field').on('click', function() {
		$(this).parents(".activity_field_box").find('.input_fields').find(".check_error").text("");
	});
	...
	// 활동요일 등록 버튼 클릭 시 에러메세지 삭제
	$(".day_result").on('click', function() {
		$(this).parents(".input_day_box").find(".check_error").text("");
	});
});